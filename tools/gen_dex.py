...
def main():
    ...
    out.append('  { "?", 0, 0, 0, 0x2946, 50, 50, 50, 50, 50, 50, TYPE_NONE, TYPE_NONE, 0, "" },  // 0: sin usar\n')
    for num, slug, display, typ, evo, lvl in DEX:
        ...
        if num in evolved:
            ...
            rar = 'R_COMUN'
        rarities.append(rar)
        hp, atk, df, spa, spd, spe = BASE_STATS[num]
        ...
        desc = DESC.get(num, '').replace('\\', '\\\\').replace('"', '\\"')
        out.append(f'  {{ "{display}", {evo}, {lvl}, {rar}, 0x{acc:04X}, {hp}, {atk}, {df}, {spa}, {spd}, {spe}, {t1}, {t2}, {bio}, "{desc}" }},  // {num} {type1}' + (f'/{type2}' if type2 else '') + '\n')
    out.append("};\n\n")
