...
struct AudioEvent {
  uint8_t kind;
  uint16_t value;  // dex-Nummer bis 65535 (SFX-IDs passen locker mit rein)
};
...
static const Note N_SLEEP[]    = {SOFT(523, 90, 56), SOFT(392, 90, 52), SL(262, 160, 196, 48, W_SOFT)};
static const Note N_WAKE[]     = {TRI(392, 60, 66), TRI(523, 60, 72), SOFT(659, 130, 74)};
...
// Spielt den echten Pokemon-Ruf von der SD (/cries/pNNN.pcm, PCM16 mono
// 16kHz ohne Header) statt eines synthetisierten Chirps. Fehlt die Datei
// (z.B. noch nicht hochgeladen), bleibt es einfach still - kein Fallback-Ton,
// damit nie ein Fantasie-Chirp und ein echter Ruf gemischt werden.
static void playSpeciesCryFromSD(uint16_t dex) {
  char path[24];
  snprintf(path, sizeof(path), "/cries/p%03d.pcm", dex);
  File f = SD_MMC.open(path, FILE_READ);
  if (!f) return;
  int16_t mono[256];
  while (f.available()) {
    int nb = f.read((uint8_t *)mono, sizeof(mono));
    if (nb <= 0) break;
    int n = nb / 2;
  ...
  for (;;) {
    ...
    if (isSfx || isChirp) {
      ...
      if (isSfx) {
        ...
        playSpeciesCryFromSD(event.value);
      }
